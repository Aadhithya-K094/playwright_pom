...
const MODULE_URL = "/component/advanced-stem";

test.use({ storageState: { cookies: [], origins: [] } });

test.describe("Component > STEM", () => {
    ...
    test("Verify STEM page loads", async ({ page }) => {
    ...
    test("Verify STEM page title", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
    ...
    test("Verify STEM navigation from Component menu", async ({ page }) => {
        ...
        // Click sub-menu item
        await page.getByRole("link", { name: "STEM" }).click();
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
    ...
    test("Verify no console errors on STEM", async ({ page }) => {
        const errors = [];
    ...
    test("Take STEM screenshot", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
        await page.screenshot({ path: "test-results/screenshots/BEO1_Component_STEM.png", fullPage: true });
    });
});
