...
const MODULE_URL = "/component/schemes";

test.use({ storageState: { cookies: [], origins: [] } });

test.describe("Component > Schemes", () => {
    ...
    test("Verify Schemes page loads", async ({ page }) => {
    ...
    test("Verify Schemes page title", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
    ...
    test("Verify Schemes navigation from Component menu", async ({ page }) => {
    ...
    test("Verify no console errors on Schemes", async ({ page }) => {
        const errors = [];
    ...
    test("Take Schemes screenshot", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
        await page.screenshot({ path: "test-results/screenshots/BEO1_Component_Schemes.png", fullPage: true });
    });
});
