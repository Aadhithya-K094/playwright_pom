...
const MODULE_URL = "/approval/event-registration-approval";

test.use({ storageState: { cookies: [], origins: [] } });

test.describe("Approvals > Event Registration", () => {
    ...
    test("Verify Event Registration page loads", async ({ page }) => {
    ...
    test("Verify Event Registration page title", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
    ...
    test("Verify Event Registration navigation from Approvals menu", async ({ page }) => {
    ...
    test("Verify no critical errors on Event Registration", async ({ page }) => {
        const errors = [];
    ...
    test("Take Event Registration screenshot", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
        await page.screenshot({ path: "test-results/screenshots/BEO1_Approvals_Event_Registration.png", fullPage: true });
    });
});
