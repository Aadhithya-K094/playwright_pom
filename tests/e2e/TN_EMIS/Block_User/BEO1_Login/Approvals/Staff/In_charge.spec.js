...
const MODULE_URL = "/approval/incharge";

test.use({ storageState: { cookies: [], origins: [] } });

test.describe("Approvals > In charge", () => {
    ...
    test("Verify In charge page loads", async ({ page }) => {
    ...
    test("Verify In charge page title", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
    ...
    test("Verify In charge navigation from Approvals menu", async ({ page }) => {
        ...
        // Click sub-menu item
        await page.getByRole("link", { name: "In charge" }).click();
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
    ...
    test("Verify no critical errors on In charge", async ({ page }) => {
        const errors = [];
    ...
    test("Take In charge screenshot", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
        await page.screenshot({ path: "test-results/screenshots/BEO1_Approvals_In_charge.png", fullPage: true });
    });
});
