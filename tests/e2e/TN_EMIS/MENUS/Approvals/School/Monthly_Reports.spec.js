...
const MODULE_URL = "/approval/sclmonthlyreports";

test.use({ storageState: { cookies: [], origins: [] } });

test.describe("Approvals > Monthly Reports", () => {
    ...
    test("Verify Monthly Reports page loads", async ({ page }) => {
    ...
    test("Verify Monthly Reports page title", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
    ...
    test("Verify Monthly Reports navigation from Approvals menu", async ({ page }) => {
    ...
    test("Verify no critical errors on Monthly Reports", async ({ page }) => {
        const errors = [];
    ...
    test("Take Monthly Reports screenshot", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
        await page.screenshot({ path: "test-results/screenshots/BEO1_Approvals_Monthly_Reports.png", fullPage: true });
    });
});
