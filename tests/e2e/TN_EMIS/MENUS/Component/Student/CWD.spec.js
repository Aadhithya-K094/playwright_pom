...
const MODULE_URL = "/dashboard/cwsnDashboard";

test.use({ storageState: { cookies: [], origins: [] } });

test.describe("Component > CWD", () => {
    ...
    test("Verify CWD page loads", async ({ page }) => {
    ...
    test("Verify CWD page title", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
    ...
    test("Verify CWD navigation from Component menu", async ({ page }) => {
        ...
        // Click sub-menu item
        await page.getByRole("link", { name: "CWD" }).click();
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
    ...
    test("Verify no console errors on CWD", async ({ page }) => {
        const errors = [];
    ...
    test("Take CWD screenshot", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
        await page.screenshot({ path: "test-results/screenshots/BEO1_Component_CWD.png", fullPage: true });
    });
});
