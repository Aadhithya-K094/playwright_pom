...
const MODULE_URL = "/component/crc-training";

test.use({ storageState: { cookies: [], origins: [] } });

test.describe("Component > Teacher Training", () => {
    ...
    test("Verify Teacher Training page loads", async ({ page }) => {
    ...
    test("Verify Teacher Training page title", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
    ...
    test("Verify Teacher Training navigation from Component menu", async ({ page }) => {
        ...
        // Click sub-menu item
        await page.getByRole("link", { name: "Teacher Training" }).click();
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
    ...
    test("Verify no console errors on Teacher Training", async ({ page }) => {
        const errors = [];
    ...
    test("Take Teacher Training screenshot", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
        await page.screenshot({ path: "test-results/screenshots/BEO1_Component_Teacher_Training.png", fullPage: true });
    });
});
