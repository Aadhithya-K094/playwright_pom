...
const MODULE_URL = "/component/staff-griev-dashboard";

test.use({ storageState: { cookies: [], origins: [] } });

test.describe("Component > Grievance", () => {
    ...
    test("Verify Grievance page loads", async ({ page }) => {
    ...
    test("Verify Grievance page title", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
    ...
    test("Verify Grievance navigation from Component menu", async ({ page }) => {
        ...
        // Click sub-menu item
        await page.getByRole("link", { name: "Grievance" }).click();
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
    ...
    test("Verify no console errors on Grievance", async ({ page }) => {
        const errors = [];
    ...
    test("Take Grievance screenshot", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
        await page.screenshot({ path: "test-results/screenshots/BEO1_Component_Grievance.png", fullPage: true });
    });
});
