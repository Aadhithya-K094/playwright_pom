...
const MODULE_URL = "/component/sports";

test.use({ storageState: { cookies: [], origins: [] } });

test.describe("Component > Sports", () => {
    ...
    test("Verify Sports page loads", async ({ page }) => {
    ...
    test("Verify Sports page title", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
    ...
    test("Verify Sports navigation from Component menu", async ({ page }) => {
    ...
    test("Verify no console errors on Sports", async ({ page }) => {
        const errors = [];
    ...
    test("Take Sports screenshot", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
        await page.screenshot({ path: "test-results/screenshots/BEO1_Component_Sports.png", fullPage: true });
    });
});
