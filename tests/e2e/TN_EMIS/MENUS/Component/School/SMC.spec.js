...
const MODULE_URL = "/component/smc";

test.use({ storageState: { cookies: [], origins: [] } });

test.describe("Component > SMC", () => {
    ...
    test("Verify SMC page loads", async ({ page }) => {
    ...
    test("Verify SMC page title", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
    ...
    test("Verify SMC navigation from Component menu", async ({ page }) => {
        ...
        // Click sub-menu item
        await page.getByRole("link", { name: "SMC" }).click();
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
    ...
    test("Verify no console errors on SMC", async ({ page }) => {
        const errors = [];
    ...
    test("Take SMC screenshot", async ({ page }) => {
        await page.goto(BASE_URL + MODULE_URL);
        await page.waitForLoadState("networkidle");
        await page.waitForTimeout(3000);
        await page.screenshot({ path: "test-results/screenshots/BEO1_Component_SMC.png", fullPage: true });
    });
});
