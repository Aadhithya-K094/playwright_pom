(function () {
  function _classCallCheck(a, n) { if (!(a instanceof n)) throw new TypeError("Cannot call a class as a function"); }

  function _defineProperties(e, r) { for (var t = 0; t < r.length; t++) { var o = r[t]; o.enumerable = o.enumerable || !1, o.configurable = !0, "value" in o && (o.writable = !0), Object.defineProperty(e, _toPropertyKey(o.key), o); } }

  function _createClass(e, r, t) { return r && _defineProperties(e.prototype, r), t && _defineProperties(e, t), Object.defineProperty(e, "prototype", { writable: !1 }), e; }

  function _toPropertyKey(t) { var i = _toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }

  function _toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }

  (self["webpackChunkTNSED_Administrators"] = self["webpackChunkTNSED_Administrators"] || []).push([["default-src_app_pages_ennum-ezhuthum-truthing_services_api_service_ts"], {
    /***/
    43054:
    /*!***********************************************************************!*\
      !*** ./src/app/pages/ennum-ezhuthum-truthing/services/api.service.ts ***!
      \***********************************************************************/

    /***/
    function _(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
      "use strict";

      __webpack_require__.r(__webpack_exports__);
      /* harmony export */


      __webpack_require__.d(__webpack_exports__, {
        /* harmony export */
        "ConnectionStatusEnum": function ConnectionStatusEnum() {
          return (
            /* binding */
            _ConnectionStatusEnum
          );
        },

        /* harmony export */
        "ApiService": function ApiService() {
          return (
            /* binding */
            _ApiService
          );
        }
        /* harmony export */

      });
      /* harmony import */


      var tslib__WEBPACK_IMPORTED_MODULE_13__ = __webpack_require__(
      /*! tslib */
      65353);
      /* harmony import */


      var _angular_core__WEBPACK_IMPORTED_MODULE_14__ = __webpack_require__(
      /*! @angular/core */
      37716);
      /* harmony import */


      var rxjs__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(
      /*! rxjs */
      26215);
      /* harmony import */


      var _ionic_angular__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(
      /*! @ionic/angular */
      80476);
      /* harmony import */


      var _angular_common_http__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(
      /*! @angular/common/http */
      91841);
      /* harmony import */


      var rxjs__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(
      /*! rxjs */
      25917);
      /* harmony import */


      var rxjs_operators__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(
      /*! rxjs/operators */
      5304);
      /* harmony import */


      var rxjs__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(
      /*! rxjs */
      66682);
      /* harmony import */


      var rxjs__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(
      /*! rxjs */
      22759);
      /* harmony import */


      var rxjs_operators__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(
      /*! rxjs/operators */
      88002);
      /* harmony import */


      var _ionic_native_network_ngx__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(
      /*! @ionic-native/network/ngx */
      75592);
      /* harmony import */


      var src_app_services_ionic_storage_ionic_storage_service__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(
      /*! src/app/services/ionic-storage/ionic-storage.service */
      81085);
      /* harmony import */


      var src_app_services_data_service__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(
      /*! src/app/services/data.service */
      52468);
      /* harmony import */


      var src_app_services_alert_service__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(
      /*! src/app/services/alert.service */
      25970);
      /* harmony import */


      var src_environments_environment__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(
      /*! src/environments/environment */
      92340); // import { NativeStorage } from '@awesome-cordova-plugins/native-storage/ngx';
      // import { environment } from 'src/environments/environment.prod';
      // import * as S3 from 'aws-sdk/clients/s3';


      var _ConnectionStatusEnum;

      (function (ConnectionStatusEnum) {
        ConnectionStatusEnum[ConnectionStatusEnum["Online"] = 0] = "Online";
        ConnectionStatusEnum[ConnectionStatusEnum["Offline"] = 1] = "Offline";
      })(_ConnectionStatusEnum || (_ConnectionStatusEnum = {}));

      var _ApiService = /*#__PURE__*/function () {
        function ApiService(toastController, httpClient, ionicstorage, network, dataService, alertService) {
          var _this = this;

          _classCallCheck(this, ApiService);

          this.toastController = toastController;
          this.httpClient = httpClient;
          this.ionicstorage = ionicstorage;
          this.network = network;
          this.dataService = dataService;
          this.alertService = alertService;
          this.emisApiUrl = src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.emisApiUrl;
          this.user_name = '10300766';
          this.pass_word = '9442@1969';
          this.token_sample = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..XLT-jkTVL79Q3uf-9tz3LJCoAGyUJ5PKIVPvwAcIrWE';
          this.mediumOfInstructionsArray = [];
          this.selectclassmultiplechoicearray = [{
            name: 'A',
            isSelected: false,
            id: 1
          }, {
            name: 'B',
            isSelected: false,
            id: 2
          }, {
            name: 'C',
            isSelected: false,
            id: 3
          }, {
            name: 'D',
            isSelected: false,
            id: 4
          }];
          this.classRadio = "";
          this.subjects = [];
          this.classTypeArray = [];
          this.classTypeSelected = 'multigrade';
          this.httpOptions = {
            headers: new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api'
            })
          };
          this.classes = []; //////////

          this.showSideBar = false;
          this.usernameSubject = new rxjs__WEBPACK_IMPORTED_MODULE_6__.BehaviorSubject('');
          this.studentAssessedArray = new rxjs__WEBPACK_IMPORTED_MODULE_6__.BehaviorSubject([]);
          this.totalStudentsAssesedClass1 = this.studentAssessedArray.asObservable();
          this.username = this.usernameSubject.asObservable();
          this.FormativeAssessmentA = [];
          this.studentsListquestionList = [];
          this.classroomDetails = [];
          this.RemainingUser = 0;
          this.showtabs = '';
          this.readingFileApi = 'https://d8omnqcdi1.execute-api.ap-south-1.amazonaws.com/staging-resource-collection-mgmt/resource-collection-mgmt/resource-listurl%22';
          this.class1Students = [{
            name: "Anitha P",
            roll: "5674873",
            score: null,
            isAssessed: false,
            isSkipped: false,
            skippedReason: ''
          }, {
            name: "Rohan S",
            roll: "5674871",
            score: null,
            isAssessed: false,
            isSkipped: false,
            skippedReason: ''
          }, {
            name: "Ram P",
            roll: "5674872",
            score: null,
            isAssessed: false,
            isSkipped: false,
            skippedReason: ''
          }, {
            name: "Ram Prasad",
            roll: "5670072",
            score: null,
            isAssessed: false,
            isSkipped: false,
            skippedReason: ''
          }, {
            name: "Rose S",
            roll: "56748702",
            score: null,
            isAssessed: false,
            isSkipped: false,
            skippedReason: ''
          }, {
            name: "Kulwinder S",
            roll: "5674874",
            score: null,
            isAssessed: false,
            isSkipped: false,
            skippedReason: ''
          }, {
            name: "Jeevan P",
            roll: "56748573",
            score: null,
            isAssessed: false,
            isSkipped: false,
            skippedReason: ''
          }, {
            name: "Reema S",
            roll: "56108573",
            score: null,
            isAssessed: false,
            isSkipped: false,
            skippedReason: ''
          }, {
            name: "praveen S",
            roll: "00108573",
            score: null,
            isAssessed: false,
            isSkipped: false,
            skippedReason: ''
          }, {
            name: "balveera S",
            roll: "00155573",
            score: null,
            isAssessed: false,
            isSkipped: false,
            skippedReason: ''
          }];
          this.Questions = [{
            title: "Which of the following ruler of Satavahana Empire composed Gathasaptashati?",
            options: ['Simuka', 'Gautamiputra Satkarni', 'Pulumayi', 'Hala'],
            answer: "Hala",
            type: "textOnly"
          }, {
            title: "What is being shown below?",
            image: "https://upload.wikimedia.org/wikipedia/commons/1/1a/Elephant_Diversity.jpg",
            options: ['Elephant', 'Tiger', 'Dog', 'Cat'],
            answer: "Elephant",
            type: "imageWithText"
          }, {
            title: "Which of the following is the Australian flag??",
            options: [{
              answerKey: 0,
              image: "https://elearning.uq.edu.au/files/10118/multiple-choice-with-images-02.png"
            }, {
              answerKey: 1,
              image: "https://elearning.uq.edu.au/files/10124/multiple-choice-with-images-03.png"
            }, {
              answerKey: 2,
              image: "https://elearning.uq.edu.au/files/10124/multiple-choice-with-images-03.png"
            }, {
              answerKey: 3,
              image: "https://elearning.uq.edu.au/files/10136/multiple-choice-with-images-05.png"
            }],
            answer: 3,
            type: "textWithImages"
          }, {
            title: "How does CIrcle look like?",
            options: [{
              answerKey: 0,
              image: "data:image/png;base64,"
            }, {
              answerKey: 1,
              image: "https://www.kidsmathgamesonline.com/images/pictures/shapes/hexagon.jpg"
            }, {
              answerKey: 2,
              image: "data:image/png;base64,"
            }, {
              answerKey: 3,
              image: "data:image/png;base64,"
            }],
            answer: 2,
            type: "textWithImages"
          }, {
            title: "which color tshirt is in below image",
            questionImage: "https://imgprd19.hobbylobby.com/e/5e/cd/e5ecd22cfe6a229cc51d6b30ef381ad1c6c064b1/700Wx700H-633792-0320.jpg",
            options: [{
              answerKey: 0,
              image: "data:image/png;base64,"
            }, {
              answerKey: 2,
              image: "data:image/png;base64,"
            }, {
              answerKey: 3,
              image: "https://www.colorcombos.com/images/colors/000000.png"
            }, {
              answerKey: 4,
              image: "https://www.colorcombos.com/images/colors/5BC236.png"
            }],
            answer: "0",
            type: "imageWithImage"
          }, {
            title: "With which among the following branches of Mathematics, Sulva Sutras are related to__?",
            options: ['Algenbra', 'Geometry', 'Calculus', 'Game Theory'],
            answer: "Geometry",
            type: "textonly"
          }, {
            title: "\n      Answer the following questions",
            questions: [{
              title: "What is 5+2?",
              options: ['6', '7', '9', '10'],
              answer: '7',
              type: "textOnly"
            }, {
              title: "What is 10 + 5?",
              options: ['3', '15', '10', '20'],
              answer: '15',
              type: "textOnly"
            }, {
              title: "What is 12 + 4?",
              options: ['3', '15', '16', '20'],
              answer: '16',
              type: "textOnly"
            }, {
              title: "What is 13 + 7?",
              options: ['3', '15', '16', '20'],
              answer: '20',
              type: "textOnly"
            }],
            type: "group"
          }, {
            title: "\n      Below are the list of animal in which you have to collectively select answers for the respective questions:\n      A. Tiger\n      B. Wolf\n      C. Cheeta\n      D. Lion\n      E. Dog",
            questions: [{
              title: "Which are the animals lives in forest?",
              options: ['A,B,C,D', 'A,B,C', 'A,C', 'B,C'],
              answer: 'A,B,C,D',
              type: "textOnly"
            }, {
              title: "Which belongs to cat family?",
              options: ['A,C', 'C,D', 'A,C,D', 'A,E'],
              answer: 'A,C,D',
              type: "textOnly"
            }, {
              title: "Which belons to Dog family?",
              options: ['E', 'B,E', 'A,B,E', 'B,C,E'],
              answer: 'B,E',
              type: "textOnly"
            }, {
              title: "Which can runs very fast?",
              options: ['B,C', 'C', 'A,B,C', 'D'],
              answer: 'B,C',
              type: "textOnly"
            }, {
              title: "Which are the animals very strong?",
              options: ['A', 'D', 'A,C,D', 'A,D'],
              answer: 'A,D',
              type: "textOnly"
            }],
            type: "group"
          }, {
            title: "\n      Please answer the following questions and tell, What below images are known as?",
            questions: [{
              image: "https://www.kidsmathgamesonline.com/images/pictures/shapes/triangle.jpg",
              options: ['Rectangle', 'Circle', 'Sqaure', 'Triangle'],
              answer: 'Triangle',
              type: "imageWithTextGroup"
            }, {
              image: "data:image/png;base64,",
              options: ['Pentagon', 'Octagon', 'Hexagon', 'Nanagon'],
              answer: 'Hexagon',
              type: "imageWithTextGroup"
            }],
            type: "group"
          }];
          this.previousStatus = _ConnectionStatusEnum.Online;
          this.ionicstorage.getData('userInfo').then(function (dataa) {
            // this.username = dataa.username;
            // this.token = dataa.token;
            _this.username = _this.user_name;
            _this.token = _this.token_sample;
          }, function (error) {
            return console.log(error);
          });
          this.ionicstorage.insertData_Replace('remainguser', {
            remainingdata: []
          });
        }

        return _createClass(ApiService, [{
          key: "usernameChanged",
          value: function usernameChanged(data) {
            this.usernameSubject.next(data);
          }
        }, {
          key: "getdata",
          value: function getdata() {
            return this.showtabs;
          }
        }, {
          key: "setdata",
          value: function setdata(val) {
            this.showtabs = val;
          }
        }, {
          key: "getdataassessed_typeid",
          value: function getdataassessed_typeid() {
            return this.assessedtypeid;
          }
        }, {
          key: "setdataassessed_typeid",
          value: function setdataassessed_typeid(val) {
            this.assessedtypeid = val;
          }
        }, {
          key: "getdataassessed_typeformativeid",
          value: function getdataassessed_typeformativeid() {
            return this.assessedtypeformativeid;
          }
        }, {
          key: "setdataassessed_typeformativeid",
          value: function setdataassessed_typeformativeid(val) {
            this.assessedtypeformativeid = val;
          }
        }, {
          key: "getdataassessed_typeformativeAid",
          value: function getdataassessed_typeformativeAid() {
            return this.assessedtypeformativeAid;
          }
        }, {
          key: "setdataassessed_typeformativeAid",
          value: function setdataassessed_typeformativeAid(val) {
            this.assessedtypeformativeAid = val;
          }
        }, {
          key: "getacadamic_yearid",
          value: function getacadamic_yearid() {
            return this.acadamic_year;
          }
        }, {
          key: "setacadamic_yearid",
          value: function setacadamic_yearid(val) {
            this.acadamic_year = val;
          }
        }, {
          key: "studentAssessedChanged",
          value: function studentAssessedChanged(data) {
            this.studentAssessedArray.next(data);
          }
        }, {
          key: "initializeNetworkEvents",
          value: function initializeNetworkEvents() {
            var _this2 = this;

            this.network.onDisconnect().subscribe(function () {
              if (_this2.previousStatus === _ConnectionStatusEnum.Online) {
                _this2.displayToast('network:offline', false);
              }

              _this2.previousStatus = _ConnectionStatusEnum.Offline;
            });
            this.network.onConnect().subscribe(function () {
              if (_this2.previousStatus === _ConnectionStatusEnum.Offline) {
                _this2.displayToast('network:online', false);
              }

              _this2.previousStatus = _ConnectionStatusEnum.Online;
            });
          }
        }, {
          key: "ngOnInit",
          value: function ngOnInit() {}
        }, {
          key: "triggerresenduser",
          value: function triggerresenduser() {
            var _this3 = this;

            this.ionicstorage.getData('remainguser').then(function (dataa) {
              _this3.RemainingUser = dataa.remainingdata;
            });
          }
        }, {
          key: "checkconnection",
          value: function checkconnection() {
            var _this4 = this;

            this.appIsOnline$.subscribe(function (online) {
              if (online) {
                _this4.NetworkConnectionCheck = 'ONLINE';
              } else {
                _this4.NetworkConnectionCheck = 'OFFLINE';
              }
            });
          }
        }, {
          key: "initConnectivityMonitoring",
          value: function initConnectivityMonitoring() {
            if (!window || !navigator || !('onLine' in navigator)) return;
            this.appIsOnline$ = (0, rxjs__WEBPACK_IMPORTED_MODULE_7__.merge)((0, rxjs__WEBPACK_IMPORTED_MODULE_8__.of)(null), (0, rxjs__WEBPACK_IMPORTED_MODULE_9__.fromEvent)(window, 'online'), (0, rxjs__WEBPACK_IMPORTED_MODULE_9__.fromEvent)(window, 'offline')).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_10__.map)(function () {
              return navigator.onLine;
            }));
          }
        }, {
          key: "displayToast",
          value: function displayToast(message, error) {
            this.toastController.create({
              header: error ? 'Error!' : 'Success!',
              message: message,
              duration: 3000,
              position: 'top',
              cssClass: error ? 'toast-custom-class-error' : 'toast-custom-class-success'
            }).then(function (toast) {
              toast.present();
            });
          }
        }, {
          key: "handleError",
          value: function handleError() {
            var _this5 = this;

            var operation = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'operation';
            var result = arguments.length > 1 ? arguments[1] : undefined;
            return function (error) {
              console.log(error);

              _this5.displayToast(error.error.msg, true);

              console.log(error);
              return (0, rxjs__WEBPACK_IMPORTED_MODULE_8__.of)(result);
            };
          }
        }, {
          key: "toggleSideBarHandler",
          value: function toggleSideBarHandler() {
            this.showSideBar = !this.showSideBar;
          }
        }, {
          key: "getQuestions",
          value: function getQuestions() {
            return this.Questions;
          }
        }, {
          key: "getAssessedStudentListClass1",
          value: function getAssessedStudentListClass1() {
            return this.totalStudentsAssesedClass1;
          }
        }, {
          key: "markAssessed",
          value: function markAssessed(id) {
            var userIndex = this.class1Students.findIndex(function (el) {
              return el.roll == id;
            });

            if (userIndex !== -1) {
              var student = this.class1Students[userIndex];
              student.isAssessed = true;
              this.class1Students.splice(userIndex, 1, student);
            }

            this.studentAssessedChanged(this.class1Students.filter(function (el) {
              return el.isAssessed != false;
            }));
          }
        }, {
          key: "markSkipped",
          value: function markSkipped(id, reason) {
            var userIndex = this.class1Students.findIndex(function (el) {
              return el.roll == id;
            });

            if (userIndex !== -1) {
              var student = this.class1Students[userIndex];

              if (reason !== 'Can be assessed later' && reason !== 'Absent today') {
                student.isAssessed = true;
              }

              student.isSkipped = true;
              student.skippedReason = reason;
              this.class1Students.splice(userIndex, 1, student);
            }

            this.studentAssessedChanged(this.class1Students.filter(function (el) {
              return el.isAssessed != false;
            }));
          }
        }, {
          key: "getStudentList",
          value: function getStudentList() {
            return this.class1Students;
          }
        }, {
          key: "getStudetnlistQuestions",
          value: function getStudetnlistQuestions(value) {
            this.studentsListquestionList = value;
          }
        }, {
          key: "saveClassRoomDetails",
          value: function saveClassRoomDetails(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "saveClassRoom", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "saveBaselineAssessmentDetails",
          value: function saveBaselineAssessmentDetails(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            var str = JSON.stringify([data]);
            var obj = {
              data: str
            };
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "submit_Assessment", obj, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "saveFromativeAssessmentDetails",
          value: function saveFromativeAssessmentDetails(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            var str = JSON.stringify([data]);
            var obj = {
              data: str
            };
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "submit_Assessment_weekly", obj, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "saveFromativeAssessment_A_Details",
          value: function saveFromativeAssessment_A_Details(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            var str = JSON.stringify([data]);
            var obj = {
              data: str
            };
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "submit_formativeA_list/", obj, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "saveSummativeAssessmentDetails",
          value: function saveSummativeAssessmentDetails(data, token) {
            debugger;
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            console.log(data.length, 'single record');

            if (data.length == undefined) {
              var str = JSON.stringify([data]);
              var obj = {
                data: str
              };
            } else {
              var _str = JSON.stringify(data);

              var obj = {
                data: _str
              };
            }

            console.log(headers, '');
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "EETAssessmentSubmit", obj, {
              headers: headers
            }).pipe();
          }
        }, {
          key: "pdfSummativeAssessmentDetails",
          value: function pdfSummativeAssessmentDetails(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "summative_pdf_generate", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getClassRoomDetails",
          value: function getClassRoomDetails(data) {
            var _this6 = this;

            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "getClassRoomDetails", data).subscribe(function (data) {
              if (data['status'] == 200 && data['dataStatus'] == true) {
                _this6.classTypeArray = data['records'].classtype;
                _this6.mediumOfInstructionsArray = data['records'].medium_instruction;

                _this6.mediumOfInstructionsArray.filter(function (val) {
                  return val.isSelected = false;
                });

                if (data['records'].academic_year_details.length > 0) {
                  _this6.setacadamic_yearid(data['records'].academic_year_details[0].id);

                  var givensubject = data['records'].academic_year_details[0].subjects_apply;
                  var seperatesubjectarray = givensubject.split(',');
                  var newsubjectarray = seperatesubjectarray.map(function (val) {
                    var findinsideidsubject = data['records'].subject.find(function (s) {
                      return s.id == val;
                    });
                    return findinsideidsubject;
                  });
                  newsubjectarray = newsubjectarray.filter(function (element) {
                    return element !== undefined;
                  });
                  _this6.subjects = newsubjectarray;
                  var givencalsses = data['records'].academic_year_details[0].classes_apply;
                  var seperatecalssarray = givencalsses.split(',');
                  var newclassarray = seperatecalssarray.map(function (val) {
                    var findinsideidclass = data['records'].students_count_list.find(function (s) {
                      return s.class_id == val;
                    });
                    return findinsideidclass;
                  });
                  newclassarray = newclassarray.filter(function (element) {
                    return element !== undefined;
                  });
                  var test = newclassarray.filter(function (value, index) {
                    var seperatesection = value.section.split(',');
                    var fintuneseperationsection = seperatesection.map(function (data, index) {
                      var obj = {
                        name: data,
                        isSelected: false,
                        id: index + 1
                      };
                      return obj;
                    });
                    value.section = fintuneseperationsection;
                    value["class"] = value.class_id;
                    value.class_id = 'Class ' + value.class_id;
                    value.classid = value.class_id.match(/\d+/)[0];
                    value.isSelected = false;
                    value.id = index + 1;
                    return value;
                  });
                  _this6.classes = test;
                } else {
                  _this6.subjects = [];
                  ['Tamil', 'English', 'Mathematics'].forEach(function (val) {
                    var getsubject = _this6.getselectivesubject(data['records'].subject, val);

                    _this6.subjects.push(getsubject);
                  });

                  var _test = data['records'].students_count_list.filter(function (value, index) {
                    if (value.class_id == "1" || value.class_id == "2" || value.class_id == "3") {
                      var seperatesection = value.section.split(',');
                      var fintuneseperationsection = seperatesection.map(function (data, index) {
                        var obj = {
                          name: data,
                          isSelected: false,
                          id: index + 1
                        };
                        return obj;
                      });
                      value.section = fintuneseperationsection;
                      value["class"] = value.class_id;
                      value.class_id = 'Class ' + value.class_id;
                      value.classid = value.class_id.match(/\d+/)[0];
                      value.isSelected = false;
                      value.id = index + 1;
                      return value;
                    }
                  });

                  _this6.classes = _test;
                }
              } else {
                _this6.alertService.error(data['msg']);
              }
            });
          }
        }, {
          key: "getselectivesubject",
          value: function getselectivesubject(array, value) {
            var findeddata = array.find(function (v) {
              return v.subject == value;
            });
            return findeddata;
          } // getUploadedFiles(bucketName, fileName, expiry) {
          //       let params = { bcktId: bucketName, filename: fileName, expiry: expiry };
          //       return this.dataService.getDataWithParamsFiles(
          //         this.readingFileApi,
          //         params,
          //         true
          //       );
          //     }

        }, {
          key: "getClassRoomDetailsAll",
          value: function getClassRoomDetailsAll(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json'
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "getClassRoomDetails", data).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getMenus",
          value: function getMenus(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "get_menuSettings", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getClassRoomCount",
          value: function getClassRoomCount(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "check_subjectCount", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getgroupQuestions",
          value: function getgroupQuestions(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "get_group_buttons", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getStudentsQuestions",
          value: function getStudentsQuestions(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "getQuestions", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getStudentsList",
          value: function getStudentsList(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "get_project_studentlist", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getStudentreport",
          value: function getStudentreport(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "get_students_term_score", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getStudentsinglereport",
          value: function getStudentsinglereport(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "get_student_report_card", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getassessedStudentsQuestions",
          value: function getassessedStudentsQuestions(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "check_submitAssessment", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getformationterms",
          value: function getformationterms(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "get_Appsetting", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getformativeAterms",
          value: function getformativeAterms(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "get_Appsetting_formative", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getformativeAllsubjects",
          value: function getformativeAllsubjects(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "get_allprojects", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "getformativeAlocatedproject",
          value: function getformativeAlocatedproject(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "get_allocated_projects", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "Saveactivityproject",
          value: function Saveactivityproject(data, token) {
            var headers = new _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpHeaders({
              'Content-Type': 'application/json',
              'Authorization': 'EMIS_web@2019_api',
              'Token': token
            });
            return this.httpClient.post(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "save_allocate_projects", data, {
              headers: headers
            }).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          } ////////Classroom details//////////////////

        }, {
          key: "onMediumOfInstructionSelection",
          value: function onMediumOfInstructionSelection(item) {
            var checkAlreadyExistingItemInArray = this.mediumOfInstructionsArray.findIndex(function (el) {
              return el.medium == item;
            });

            if (checkAlreadyExistingItemInArray !== -1) {
              var element = this.mediumOfInstructionsArray[checkAlreadyExistingItemInArray];
              element.isSelected = !element.isSelected;
              this.mediumOfInstructionsArray.splice(checkAlreadyExistingItemInArray, 1, element);
            }
          }
        }, {
          key: "selectedClassChoice",
          value: function selectedClassChoice(item, choice) {
            var findarray = this.classes.find(function (val) {
              return val.class_id == item;
            });
            findarray.section.filter(function (s) {
              var gets = choice.some(function (v) {
                return v == s.name;
              });
              s.isSelected = gets;
            });
          }
        }, {
          key: "onSelectedClassRadio",
          value: function onSelectedClassRadio(item) {
            this.classRadio = item;
          }
        }, {
          key: "onClassSelection",
          value: function onClassSelection(item) {
            var checkAlreadyExistingItemInArray = this.classes.findIndex(function (el) {
              return el.class_id == item;
            });

            if (checkAlreadyExistingItemInArray !== -1) {
              var element = this.classes[checkAlreadyExistingItemInArray];
              element.isSelected = !element.isSelected;
              this.classes.splice(checkAlreadyExistingItemInArray, 1, element);
            }
          }
        }, {
          key: "onClassSelectionformongrade",
          value: function onClassSelectionformongrade(item) {
            var checkAlreadyExistingItemInArray = this.classes.findIndex(function (el) {
              return el.class_id == item;
            });

            if (checkAlreadyExistingItemInArray !== -1) {
              var element = this.classes[checkAlreadyExistingItemInArray];
              element.isSelected = !element.isSelected;
              this.classes.splice(checkAlreadyExistingItemInArray, 1, element);
            }
          }
        }, {
          key: "onSubjectSelection",
          value: function onSubjectSelection(item) {
            var checkAlreadyExistingItemInArray = this.subjects.findIndex(function (el) {
              return el.subject == item;
            });

            if (checkAlreadyExistingItemInArray !== -1) {
              var element = this.subjects[checkAlreadyExistingItemInArray];
              element.isSelected = !element.isSelected;
              this.subjects.splice(checkAlreadyExistingItemInArray, 1, element);
            }
          }
        }, {
          key: "onClassTypeSelection",
          value: function onClassTypeSelection(item) {
            this.classTypeSelected = item;
          } ///////////////////////////////////////////

        }, {
          key: "toGetStudentList",
          value: function toGetStudentList(data) {
            return this.dataService.post(this.emisApiUrl + '/EETStudentList', data).map(function (res) {
              return res;
            });
          }
        }, {
          key: "eet_error_log",
          value: function eet_error_log(data) {
            return this.dataService.post(this.emisApiUrl + '/eet_error_log', data).map(function (res) {
              return res;
            });
          }
        }, {
          key: "eet_school_observation",
          value: function eet_school_observation() {
            // let headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': 'EMIS_web@2019_api', 'Token': token });
            return this.dataService.post(this.emisApiUrl + '/get_eet_schools', true).map(function (res) {
              return res;
            });
          }
        }, {
          key: "getUdiseSchool",
          value: function getUdiseSchool(data) {
            // let headers = new HttpHeaders({ 'Content-Type': 'application/json', 'Authorization': 'EMIS_web@2019_api', 'Token': token });
            return this.httpClient.get(src_environments_environment__WEBPACK_IMPORTED_MODULE_4__.environment.api_url + "InfoByUDISE?udise=" + data).pipe((0, rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.catchError)(this.handleError('Error occured')));
          }
        }, {
          key: "createObservation",
          value: function createObservation(data) {
            return this.dataService.post(this.emisApiUrl + '/eet_create_observation', data).map(function (res) {
              return res;
            });
          }
        }, {
          key: "eet_updateObservation",
          value: function eet_updateObservation(data) {
            return this.dataService.post(this.emisApiUrl + '/eet_update_observation', data).map(function (res) {
              return res;
            });
          }
        }]);
      }();

      _ApiService.ctorParameters = function () {
        return [{
          type: _ionic_angular__WEBPACK_IMPORTED_MODULE_12__.ToastController
        }, {
          type: _angular_common_http__WEBPACK_IMPORTED_MODULE_5__.HttpClient
        }, {
          type: src_app_services_ionic_storage_ionic_storage_service__WEBPACK_IMPORTED_MODULE_1__.IonicStorageService
        }, {
          type: _ionic_native_network_ngx__WEBPACK_IMPORTED_MODULE_0__.Network
        }, {
          type: src_app_services_data_service__WEBPACK_IMPORTED_MODULE_2__.DataService
        }, {
          type: src_app_services_alert_service__WEBPACK_IMPORTED_MODULE_3__.AlertService
        }];
      };

      _ApiService = (0, tslib__WEBPACK_IMPORTED_MODULE_13__.__decorate)([(0, _angular_core__WEBPACK_IMPORTED_MODULE_14__.Injectable)({
        providedIn: 'root'
      })], _ApiService);
      /***/
    }
  }]);
})();
//# sourceMappingURL=default-src_app_pages_ennum-ezhuthum-truthing_services_api_service_ts-es5.js.map