  ...
  (self["webpackChunkTNSED_Administrators"] = self["webpackChunkTNSED_Administrators"] || []).push([["src_app_pages_pmoa_spectacel_class-list_class-list_module_ts"], {
    ...
    function _(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
      ...
      var _ClassListPage = /*#__PURE__*/function () {
        function ClassListPage(router, healthServices, route, sqliteDB) {
        ...
        return _createClass(ClassListPage, [{
          ...
          key: "nextPage",
          value: function nextPage(classId, SchoolId, classSection) {
            this.router.navigate(['/tabs/spectacel-section-list'], {
              queryParams: {
                classId: classId,
                SchoolId: SchoolId,
                classSection: JSON.stringify(classSection)
              }
            });
          }
        }, {
          key: "studentPage",
          value: function studentPage(class_studying_id, class_section, SchoolId) {
          ...
          value: function getTotalClassData() {
            ...
            this.healthServices.StudRefSpecs(this.schoolId).subscribe(function (response) {
              _this2.totalstds = response.result[0].total;
              _this2.notRec = response.result[0].s1;
              _this2.verifiedRecd = response.result[0].s2;
              _this2.defRet = response.result[0].s3;
              _this2.corVerifiedRecd = response.result[0].s4;
              _this2.toBeVerified = parseInt(response.result[0].total) - Number(response.result[0].verified);
            });
          }
        }, {
          key: "getClassSecData",
          value: function getClassSecData() {
            ...
            this.healthServices.SpecsNeedClsList(this.schoolId).subscribe(function (response) {
              _this3.classSecListData = []; // Grouping by class_studying_id

              var groupedData = response.result.reduce(function (acc, item) {
                var existing = acc.find(function (obj) {
                  return obj.class_studying_id === item.class_studying_id;
                });
              ...
              _this3.classSecListData = groupedData;
          ...
          value: function onSync() {
            var query = 'DELETE FROM  ClassSec'; // if (data.rowsAffected > 0) {
            //   this.getBlocklistServer();
            // }

            this.getTotalClassData();
            this.getClassSecData(); //     return this.sqliteDB.getDataLocalDB(query).then(data1 => {
      ...
      _ClassListPage = (0, tslib__WEBPACK_IMPORTED_MODULE_5__.__decorate)([(0, _angular_core__WEBPACK_IMPORTED_MODULE_6__.Component)({
...
//# sourceMappingURL=src_app_pages_pmoa_spectacel_class-list_class-list_module_ts-es5.js.map