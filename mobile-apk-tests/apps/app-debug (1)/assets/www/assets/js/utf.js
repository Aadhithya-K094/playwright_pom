...
function changeText(e,currField,numchar)
{
    if (!isIE)
	    Key = e.which;
    else
	...
	else
	{
		...
		if( myField.selectionStart >= 0)
		{
			...
			{
				prevChar = "";
				currField.value = getLang(prevChar,Char, startPos)
			}
			else
			{
				prevChar = myField.value.substring(startPos - 1,startPos);
				...
				currField.value = prevStr + getLang(prevChar,Char, myField.selectionStart)
						  + myField.value.substring(endPos, myField.value.length);
			}
			myField.selectionStart = sPos ;
			myField.selectionEnd = sPos;
